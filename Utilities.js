...
var aggregatorTemplates = {
  ...
  fractionOf: function fractionOf(wrapped) {
    ...
    return function () {
      ...
      return function (data, stubKey, headerKey) {
        return {
          selector: {
            total: [[], []],
            stub: [stubKey, []],
          ...
          inner: wrapped.apply(undefined, _toConsumableArray(Array.from(x || [])))(data, stubKey, headerKey),
          push: function push(record) {
...
var aggregators = function (tpl) {
  return {
    ...
    'Count as Fraction of Columns': tpl.fractionOf(tpl.count(), 'header', usFmtPct)
  };
}(aggregatorTemplates);
...
var derivers = {
  bin: function bin(header, binWidth) {
    return function (record) {
      return record[header] - record[header] % binWidth;
    };
  },
  dateFormat: function dateFormat(header, formatString) {
    ...
    return function (record) {
      var date = new Date(Date.parse(record[header]));
      if (isNaN(date)) {
        return '';
      }
  ...
  _createClass(PivotData, [{
    ...
    value: function filter(record) {
      for (var k in this.props.valueFilter) {
        var obj = this.props.valueFilter[k];
    ...
    value: function sortKeys() {
      ...
      if (!this.sorted) {
        ...
        switch (this.props.stubOrder) {
          case 'value_a_to_z':
            this.stubKeys.sort(function (a, b) {
        ...
        switch (this.props.headerOrder) {
          case 'value_a_to_z':
            this.headerKeys.sort(function (a, b) {
              return naturalSort(v([], a), v([], b));
            });
            break;
          case 'value_z_to_a':
    ...
    key: 'getheaderKeys',
    value: function getheaderKeys() {
      this.sortKeys();
      return this.headerKeys;
    }
  }, {
    key: 'getStubKeys',
    value: function getStubKeys() {
      this.sortKeys();
    ...
    value: function processRecord(record) {
      ...
      try {
        for (var _iterator3 = Array.from(this.props.headers)[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
          var x = _step3.value;
      ...
      try {
        for (var _iterator4 = Array.from(this.props.stubs)[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
          var _x16 = _step4.value;

          stubKey.push(_x16 in record ? record[_x16] : 'null');
        }
      } catch (err) {
      ...
      var flatStubKey = stubKey.join(String.fromCharCode(0));
      var flatheaderKey = headerKey.join(String.fromCharCode(0));
      ...
      if (headerKey.length !== 0 && stubKey.length !== 0) {
        ...
        if (!this.tree[flatStubKey][flatheaderKey]) {
          this.tree[flatStubKey][flatheaderKey] = this.aggregator(this, stubKey, headerKey);
        }
    ...
    key: 'getAggregator',
    value: function getAggregator(stubKey, headerKey) {
      var agg = void 0;
      var flatStubKey = stubKey.join(String.fromCharCode(0));
      var flatheaderKey = headerKey.join(String.fromCharCode(0));
...
PivotData.propTypes = {
  ...
  stubs: _propTypes2.default.arrayOf(_propTypes2.default.string),
  ...
  stubValueFilter: _propTypes2.default.objectOf(_propTypes2.default.objectOf(_propTypes2.default.bool)),
  headerValueFilter: _propTypes2.default.objectOf(_propTypes2.default.objectOf(_propTypes2.default.bool)),
  ...
  stubOrder: _propTypes2.default.oneOf(['key_a_to_z', 'value_a_to_z', 'value_z_to_a']),
  headerOrder: _propTypes2.default.oneOf(['key_a_to_z', 'value_a_to_z', 'value_z_to_a'])
};
